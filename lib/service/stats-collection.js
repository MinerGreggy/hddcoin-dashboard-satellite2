...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'hddcoin-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('hddcoin').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('hddcoin')[service]);
